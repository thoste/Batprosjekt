...
/*
"createPumpTable" printer ut en tabell med data for pumpe og vannhøyde fra tabellen $boatname i databasen.
...
Brukes ikke i mainExample.php men kan være nyttig.
*/
function createPumpTable($conn, $boatname,$timeRange,$column){
	$dateTimeStr0 = dateTimeStrToUnix($timeRange[0]);
	$dateTimeStr1 = dateTimeStrToUnix($timeRange[1]);

	$sql = "SELECT $column,pumpe,timestamp FROM $boatname WHERE $column IS NOT NULL AND timestamp BETWEEN '$dateTimeStr0' AND '$dateTimeStr1'";
	...
	if($result->num_rows > 0){
		while($row = $result->fetch_assoc()){
			...
			$dateTimeStrX = gmdate("Y-m-d H:i:s", $row['timestamp']);
			echo "	<tr><td>{$dateTimeStrX}</td>
...
function printGraph($conn, $boatname, $filename, $timeRange, $column, $graphtype){
	...
	$sql = "SELECT $column, timestamp FROM $boatname WHERE $column IS NOT NULL AND timestamp BETWEEN '$dateTimeStr0' AND '$dateTimeStr1'";
	$result = $conn->query($sql);	
	$time =array();
	$yval =array();
	if($result->num_rows > 0){
	while($row=$result->fetch_assoc()){
		if($row["timestamp"] != NULL){//VIKTIG! Hvis du prøver å plotte med nullpekere på x-aksen vil JPGraph krasje og føre til store minnelekkasjer.
			$UnixTime = $row["timestamp"];
			array_push($time,gmdate("Y-m-d H:i:s",$UnixTime)); //gmdate konverterer UnixTime til en dato-streng.
